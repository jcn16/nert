    ...
    for val_batch in dataloader:
        ...
        for i in range(0, B, hparams.chunk):
            ...
            if hparams.N_importance > 0:  # sample points for fine model
                ...
                xyz_fine_sampled.requires_grad = True
                xyz_fine_sampled.retain_grad()
                _, weights, sigmas, _=inference(fine_Nerf_sigma, embedding_xyz, xyz_fine_sampled, rays_d,z_vals, weights_only=False)
                o_shared.zero_grad()
                sigmas.backward(torch.ones_like(sigmas))
                normal = F.normalize(xyz_fine_sampled.grad, p=2, dim=1)
                ture_normal=torch.sum(weights[:,:,None]*normal,-2)
                ture_normal=F.normalize(ture_normal, p=2, dim=1)

                normals.append(ture_normal.detach().cpu().numpy())
        ...
        print(path+'_normal_integral.png')
        cv2.imwrite(path+'_normal_integral.png',image_normal[:,:,::-1])
