...
def render_rays(corase_sigma,
    ...
    def inference(nerf_sigma, nerf_albedo, embedding_xyz, xyz_, embedding_dir,dir_, z_vals, weights_only=False):
        """
        ...
        dir_embedded = embedding_dir(dir_)
        if not weights_only:
        ...
        for i in range(0, B, chunk):
            ...
            if not weights_only:
                p_rgbs = nerf_albedo(dir_embedded[i:i+chunk],features)
    ...
    # for test
    rgb_coarse, depth_coarse, weights_coarse, _, _= \
        inference(corase_sigma, corase_albedo, embedding_xyz, xyz_coarse_sampled, embedding_dir,rays_d,
    ...
    if N_importance > 0:  # sample points for fine model
        ...
        rgb_fine, depth_fine, weights_fine, sigmas, visibility_fine = \
            inference(fine_sigma, fine_albedo, embedding_xyz, xyz_fine_sampled, embedding_dir,rays_d,
                      z_vals, weights_only=False)
