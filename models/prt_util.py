...
def sampleSphericalDirections(n):
    ...
    theta = np.arccos(1-2 * xv)#range 0-2*pi
    phi = 2.0 * math.pi * yv
...
def computePRT_(mesh_path, n, order):  # n=number
    mesh = trimesh.load(mesh_path, process=False)
    vectors_orig, phi, theta = sampleSphericalDirections(n)  # [n*n,3]
    SH_orig = getSHCoeffs(order, phi, theta)  # [n*n,9]

    w = 4.0 * math.pi / (n * n)

    origins = mesh.vertices  # [N,3]
    normals = mesh.vertex_normals
    n_v = origins.shape[0]  # number vertex

    origins = np.repeat(origins[:, None], n, axis=1).reshape(-1, 3)
    normals = np.repeat(normals[:, None], n, axis=1).reshape(-1, 3)  # [n_v*n,3]
    PRT_all = None
    for i in tqdm(range(n)):
        SH = np.repeat(SH_orig[None, (i * n):((i + 1) * n)], n_v, axis=0).reshape(-1, SH_orig.shape[
            1])  # shape=[n_v,n,9]->extend a para for every point
        vectors = np.repeat(vectors_orig[None, (i * n):((i + 1) * n)], n_v, axis=0).reshape(-1,
                                                                                            3)  # shape=[n_v,n,3], direction of input light
...
def computePRT(mesh_path, n, order):#n=number
    mesh = trimesh.load(mesh_path, process=False)
    vectors_orig, phi, theta = sampleSphericalDirections(n)#[n*n,3]
    SH_orig = getSHCoeffs(order, phi, theta)#[n*n,9]
    ...
    PRT_all = None
    for i in tqdm(range(n)):
        SH = np.repeat(SH_orig[None,(i*n):((i+1)*n)], n_v, axis=0).reshape(-1,SH_orig.shape[1])#shape=[n_v,n,9]->extend a para for every point
        vectors = np.repeat(vectors_orig[None,(i*n):((i+1)*n)], n_v, axis=0).reshape(-1,3)#shape=[n_v,n,3], direction of input light
