...
# models
from models.nert import Embedding,NeRF_SmoothTransport
...
TransNet=NeRF_SmoothTransport().cuda()
TransNet=torch.nn.DataParallel(TransNet)

try:
    TransNet.load_state_dict(torch.load(''))
    print('Loading sigma ...')
except:
...
for epoch in range(10000):
    ...
    #all losse
    t0=time.time()
    sub_pbar=tqdm(total=len(train_dataset))
    ...
    for batch in train_data_loader:
        ...
        surface_points=batch['surfaces'][0].cuda()
        ...
        p_normal=inference(TransNet,embedding_xyz,surface_points)
        p_normal_disturb=inference(TransNet,embedding_xyz,surface_points_disturb)

        # Compute loss
        MSE_Loss=mse_loss(p_normal,gt_transports)
    ...
    with torch.no_grad():
        ...
        Normals=inference(TransNet,embedding_xyz,surface_points)
        Normals=(0.5*Normals+0.5).view(800,800,9).permute(2,0,1)*255.0
        # gt_normals=gt_normals.view(800,800,9).permute(2,0,1)*255.0

        all_normals=[]
        for i in range(9):
            all_normals.append(torch.repeat_interleave(torch.clamp(Normals[i:i+1,:,:],0,255),3,0))
        show_image=torch.cat(all_normals,dim=2)
    ...
    torch.save(TransNet.state_dict(), model_path)
