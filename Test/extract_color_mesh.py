...
def get_opts():
    ...
    #                     help='pretrained checkpoint path to load')ex
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        ...
        for i in tqdm(range(0, B, args.chunk)):
            ...
            sigma,features=nerf_fine_sigma(xyz_embedded)
            rgbs=nerf_fine_albedo(dir_embedded,features)
    ...
    # print(vertices[:,0].max(),vertices[:,0].min())
    # print(vertices[:,1].max(),vertices[:,1].min())
    # print(vertices[:,2].max(),vertices[:,2].min())
    ...
    save_obj_mesh('meshs/points.obj', vertices)
    ...
    def save_obj_mesh_face(mesh_path, verts, faces):
        file = open(mesh_path, 'w')
    ...
    save_obj_mesh_face('meshs/pifu.obj', vertices,face)
    ...
    if args.use_vertex_normal: ## use normal vector method as suggested by the author.
        ...
        results = f([nerf_fine_sigma, nerf_fine_albedo], embeddings,
